...
#
#
def setOverRides( request, override_field, override_value):
    overrides = request.getElement("overrides")
    if type(override_field) == type([]):
        for fld, val in zip(override_field, override_value):
            ovr = overrides.appendElement()
            ovr.setElement("fieldId", fld)
            ovr.setElement("value", val)
    else:
        ovr = overrides.appendElement()
        ovr.setElement("fieldId", override_field)
        ovr.setElement("value", override_value)
    #
    return request

#
def bbRequest( ticker, field, override_field=None , override_value=None ):
    ...
    # set override fields
    if override_field:
        request = setOverRides( request, override_field, override_value)
            
    
    #overrides = ovr.AppendElement()
...
def bbBulkRequest( ticker, field, override_field=None , override_value=None ):
    request = refDataService.createRequest("ReferenceDataRequest")
    # if the ticker is a list of tickers add request to each
    if type(ticker) == type([]):
        for symbol in ticker:
...
def bbHistRequest( ticker, field, override_field=None , override_value=None , **hargs):
...
if __name__ == '__main__':
    ...
    d = bbRequest(["IBM Equity", "ASC LN Equity"], ["PX_LAST","NEWS_HEAT_STORY_FLOW_RT","LATEST_ANNOUNCEMENT_DT"])
    print d

    d = bbRequest(["IBM Equity", "ASC LN Equity"], ["LATEST_ANNOUNCEMENT_DT", "LATEST_PERIOD_END_DT_FULL_RECORD"])
    print d
    
    d = bbRequest(["IBM Equity", "ASC LN Equity"], ["LATEST_ANNOUNCEMENT_DT", "LATEST_PERIOD_END_DT_FULL_RECORD"],
                  override_field="FUND_PER", override_value="S")
    print d
